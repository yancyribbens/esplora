...
enum Commands {
    ...
    GetScriptHashTransactions { script_bytes: String, last_seen: Option<String> },
...
fn main() {
    ...
    match &cli.command {
        ...
        Commands::GetTxAtBlockIndex { block_hash, index } => {
            ...
            let r = blocking_client.get_txid_at_block_index(&hash, i).unwrap().unwrap();
            println!("{:#?}", r);
        },
        ...
        Commands::GetOutputStatus { id, index } => {
            ...
            let r = blocking_client.get_output_status(&tx_id, i).unwrap().unwrap();
            println!("{:#?}", r);
        },
        /// Broadcast transaction
        ...
        Commands::GetScriptHashTransactions { script_bytes, last_seen } => {
            let txid:Option<Txid> = None;
            if let Some(t) = last_seen {
                let txid = Some(Txid::from_str(t).unwrap());
            }
